...
class DateValidator(APIValidator):
    ...
    def validate(self, value, attr):
        if not value:
            raise ResourceException(400, 'bad_'+attr.key, "The %s field is mandatory." % attr.key)
        try:
            datetime.strptime(value, '%Y-%m-%d')
        except ValueError:
            raise ResourceException(400, 'bad_'+attr.key, 'The date %s is invalid for the %s field. Dates must be in YYYY-mm-dd form.' % (value, attr.key))
